...
seq = 0
def test(steps, sid=None):
    global seq; seq += 1
    s = sid or ("s%d" % seq)
    ok = True
    detail = ""
    for msg, checks in steps:
...
tests = [
    # 1-3: Guest best deals (with confirmation)
    ([("I have Running Shoes and Yoga Mat", ["Is that correct?"]),
      ("yes", ["Runner's Pack", "17.25"])],),
    ([("I have Yoga Mat Water Bottle Resistance Bands", ["Is that correct?"]),
      ("yes", ["Recovery Kit", "7.20"])],),
    ([("I have Resistance Bands Foam Roller Water Bottle", ["Is that correct?"]),
      ("yes", ["Recovery Kit", "7.80"])],),
    # 4-6: User IDs + name greeting (with confirmation)
    ([("user_001 Running Shoes Protein Powder Wireless Earbuds", ["Is that correct?"]),
      ("yes", ["Alice", "PLAT50"])],),
    ([("user_050 Yoga Mat Water Bottle Resistance Bands", ["Is that correct?"]),
      ("yes", ["Zara", "Recovery Kit"])],),
    ([("user_450 Running Shoes", ["Is that correct?"]),
      ("yes", ["Zion", "Runner's Pack"])],),
    ...
    ([("user_001 what deals for Running Shoes", ["Is that correct?"]),
      ("yes", ["Alice", "LOYAL20"])], "follow1"),
    ...
    # 26: Guest single item (with confirmation)
    ([("I have Running Shoes and Yoga Mat get deals", ["Is that correct?"]),
      ("yes", ["Runner's Pack"])],),
    # 27: User asking about cart items
...
for i, args in enumerate(tests, 1):
    ...
    if ok:
        ...
        failed += 1
        status = "FAIL"
    print("  %s #%2d: %-55s | %.120s" % (status, i, steps[0][0][:55], detail))
